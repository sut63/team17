import privateExports__default, { ApiRegistry, defaultSystemIcons, useApi, alertApiRef, errorApiRef, storageApiRef, oauthRequestApiRef, appThemeApiRef, SessionState, identityApiRef, configApiRef, googleAuthApiRef, githubAuthApiRef, gitlabAuthApiRef, oktaAuthApiRef, oauth2ApiRef, useApiHolder } from '@backstage/core-api';
export * from '@backstage/core-api';
import React, { useState, useEffect, useRef, useCallback, useLayoutEffect, useMemo, Component as Component$3, Children, isValidElement, useContext, Fragment, forwardRef, createContext } from 'react';
import { BrowserRouter, MemoryRouter, Link as Link$2, NavLink } from 'react-router-dom';
import { makeStyles, Grid, Typography, Link as Link$1, LinearProgress, Snackbar, IconButton, Button as Button$1, useTheme, Tooltip, ClickAwayListener, ListItem, ListItemAvatar, ListItemText, Dialog, DialogTitle, DialogContent, List, DialogActions, Divider, ListItemIcon, withStyles, CardHeader, CardActions, Card, CardContent, Stepper, Step, StepLabel, StepContent, TableCell, createStyles, Table as Table$1, TableBody, TableRow, Popover, Tab, Tabs as Tabs$1, AppBar, Chip, Badge, TextField, styled, Collapse, Avatar, FormControl, FormHelperText } from '@material-ui/core';
...
class StructuredMetadataTable extends Component$3 {
  render() {
...
const useStyles$m = (props) => makeStyles((theme) => ({
  container: {
    width: "100%",
    display: "flex",
    flexDirection: "row",
...
const ContentHeader = ({
  ...
  const classes = useStyles$m({textAlign})();
...
function genTheme(colors, shape) {
...
const PageThemeContext = React.createContext(pageTheme.home);
...
const Page = ({theme = pageTheme.home, children}) => {
  ...
  return /* @__PURE__ */ React.createElement(PageThemeContext.Provider, {
    value: theme
  }, /* @__PURE__ */ React.createElement("div", {
    className: classes.root
  }, children));
...
const Header = ({
  ...
  const theme2 = useContext(PageThemeContext);
  const classes = useStyles$o({backgroundImage: theme2.backgroundImage});
...
const HeaderTabs = ({tabs, onChange, selectedIndex}) => {
...
const SidebarPage = (props) => {
  ...
  const classes = useStyles$s({isPinned});
...
const useStyles$t = makeStyles((theme2) => ({
  ...
  drawer: {
    ...
    bottom: 0,
    padding: 0,
    background: theme2.palette.navigation.background,
    overflowX: "hidden",
...
const useStyles$u = makeStyles((theme2) => {
  ...
  return {
    ...
    selected: {
      "&$root": {
        borderLeft: `solid ${selectedIndicatorWidth}px ${theme2.palette.navigation.indicator}`,
        color: "#ffffff"
      },
      "&$closed": {
...
const SidebarPinButton = () => {
  ...
  const classes = useStyles$w({isPinned});
  return /* @__PURE__ */ React.createElement("div", {
    className: classes.root
...
function SidebarUserSettings() {
  ...
  const providers = (_a = providersConfig == null ? void 0 : providersConfig.keys()) != null ? _a : [];
  useEffect(() => {
    if (!sidebarOpen && open)
      setOpen(false);
  }, [open, sidebarOpen]);
  return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(UserProfile, {
    ...
    in: open,
    timeout: "auto"
  }, providers.includes("google") && /* @__PURE__ */ React.createElement(OIDCProviderSettings, {
    title: "Google",
    apiRef: googleAuthApiRef,
    icon: Star2
  }), providers.includes("github") && /* @__PURE__ */ React.createElement(OAuthProviderSettings, {
    title: "GitHub",
    apiRef: githubAuthApiRef,
    icon: Star2
  }), providers.includes("gitlab") && /* @__PURE__ */ React.createElement(OAuthProviderSettings, {
    title: "GitLab",
    apiRef: gitlabAuthApiRef,
    icon: Star2
  }), providers.includes("okta") && /* @__PURE__ */ React.createElement(OIDCProviderSettings, {
    title: "Okta",
    apiRef: oktaAuthApiRef,
    icon: Star2
  }), providers.includes("oauth2") && /* @__PURE__ */ React.createElement(OIDCProviderSettings, {
    title: "YourOrg",
    apiRef: oauth2ApiRef,
    icon: Star2
  }), /* @__PURE__ */ React.createElement(SidebarItem, {
...
const SignInPage = ({
  ...
  const configApi = useApi(configApiRef);
  const classes = useStyles$y();
  const signInProviders = getSignInProviders(providers2);
