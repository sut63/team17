...
import { createApiFactory, alertApiRef, AlertApiForwarder, errorApiRef, ErrorAlerter, ErrorApiForwarder, oauthRequestApiRef, OAuthRequestManager, googleAuthApiRef, GoogleAuth, githubAuthApiRef, GithubAuth, gitlabAuthApiRef, GitlabAuth, auth0AuthApiRef, Auth0Auth, createApp, SidebarItem, Sidebar, SidebarSpacer, ApiTestRegistry, AlertDisplay, OAuthRequestDialog, SidebarPage } from '@backstage/core';
...
const googleAuthApiFactory = createApiFactory({
  ...
  factory: ({oauthRequestApi}) => GoogleAuth.create({
...
const githubAuthApiFactory = createApiFactory({
  ...
  deps: {oauthRequestApi: oauthRequestApiRef},
  factory: ({oauthRequestApi}) => GithubAuth.create({
    backendUrl: "http://localhost:7000",
    basePath: "/auth/",
    oauthRequestApi
  })
...
const gitlabAuthApiFactory = createApiFactory({
  ...
  deps: {oauthRequestApi: oauthRequestApiRef},
  factory: ({oauthRequestApi}) => GitlabAuth.create({
    backendUrl: "http://localhost:7000",
    basePath: "/auth/",
    oauthRequestApi
  })
...
const auth0AuthApiFactory = createApiFactory({
  ...
  deps: {oauthRequestApi: oauthRequestApiRef},
  factory: ({oauthRequestApi}) => Auth0Auth.create({
    backendUrl: "http://localhost:7000",
    basePath: "/auth/",
    oauthRequestApi
  })
