...
var tasks = require('./tasks-9d1dd6fb.js');
require('ora');
...
var createPlugin = async () => {
  ...
  const {version} = await fs.readJson(index.paths.resolveTargetRoot("lerna.json"));
  tasks.Task.log();
  ...
  try {
    ...
    await tasks.templatingTask(templateDir, tempDir, {...answers, version});
    ...
    if (await fs.pathExists(appPackage)) {
      ...
      await addPluginDependencyToApp(index.paths.targetRoot, answers.id, version);
      tasks.Task.section("Import plugin in app");
      await addPluginToApp(index.paths.targetRoot, answers.id);
    }
    if (ownerIds && ownerIds.length) {
      await tasks.addCodeownersEntry(codeownersPath, `/plugins/${answers.id}`, ownerIds);
    }
    tasks.Task.log();
    tasks.Task.log(`🥇  Successfully created ${chalk2.cyan(`@backstage/plugin-${answers.id}`)}`);
    tasks.Task.log();
    tasks.Task.exit();
  } catch (error) {
    tasks.Task.error(error.message);
    tasks.Task.log("It seems that something went wrong when creating the plugin 🤔");
    tasks.Task.log("We are going to clean up, and then you can try again.");
    tasks.Task.section("Cleanup");
    await cleanUp(tempDir);
    tasks.Task.error("🔥  Failed to create plugin!");
    tasks.Task.exit(1);
  }
};
