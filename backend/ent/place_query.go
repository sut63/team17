...
// QueryPlaceActi chains the current query on the place_acti edge.
func (pq *PlaceQuery) QueryPlaceActi() *ActivityQuery {
	query := &ActivityQuery{config: pq.config}
...
//  WithPlaceActi tells the query-builder to eager-loads the nodes that are connected to
// the "place_acti" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PlaceQuery) WithPlaceActi(opts ...func(*ActivityQuery)) *PlaceQuery {
	query := &ActivityQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPlaceActi = query
	return pq
}
...
func (pq *PlaceQuery) sqlAll(ctx context.Context) ([]*Place, error) {
	...
	if query := pq.withPlaceActi; query != nil {
		...
		query.Where(predicate.Activity(func(s *sql.Selector) {
			s.Where(sql.InValues(place.PlaceActiColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.place_place_acti
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "place_place_acti" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "place_place_acti" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.PlaceActi = append(node.Edges.PlaceActi, n)
		}
	}
