...
// HasRegiProv applies the HasEdge predicate on the "regi_prov" edge.
func HasRegiProv() predicate.Region {
	return predicate.Region(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(RegiProvTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, RegiProvTable, RegiProvColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasRegiProvWith applies the HasEdge predicate on the "regi_prov" edge with a given conditions (other predicates).
func HasRegiProvWith(preds ...predicate.Province) predicate.Region {
	return predicate.Region(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(RegiProvInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, RegiProvTable, RegiProvColumn),
		)
