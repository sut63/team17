...
// QueryCounProv chains the current query on the coun_prov edge.
func (cq *CountryQuery) QueryCounProv() *ProvinceQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(country.Table, country.FieldID, cq.sqlQuery()),
...
//  WithCounProv tells the query-builder to eager-loads the nodes that are connected to
// the "coun_prov" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CountryQuery) WithCounProv(opts ...func(*ProvinceQuery)) *CountryQuery {
	query := &ProvinceQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCounProv = query
	return cq
}
...
func (cq *CountryQuery) sqlAll(ctx context.Context) ([]*Country, error) {
	...
	if query := cq.withCounProv; query != nil {
		...
		query.Where(predicate.Province(func(s *sql.Selector) {
			s.Where(sql.InValues(country.CounProvColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.country_coun_prov
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "country_coun_prov" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "country_coun_prov" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.CounProv = append(node.Edges.CounProv, n)
		}
	}
