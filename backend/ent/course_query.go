...
type CourseQuery struct {
	...
	// eager-loading edges.
	withCourFacu *FacultyQuery
	withCourDegr *DegreeQuery
	withCourInst *InstitutionQuery
	withFKs      bool
...
// QueryCourFacu chains the current query on the cour_facu edge.
func (cq *CourseQuery) QueryCourFacu() *FacultyQuery {
	query := &FacultyQuery{config: cq.config}
...
// QueryCourDegr chains the current query on the cour_degr edge.
func (cq *CourseQuery) QueryCourDegr() *DegreeQuery {
	query := &DegreeQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryCourInst chains the current query on the cour_inst edge.
func (cq *CourseQuery) QueryCourInst() *InstitutionQuery {
	query := &InstitutionQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithCourFacu tells the query-builder to eager-loads the nodes that are connected to
// the "cour_facu" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CourseQuery) WithCourFacu(opts ...func(*FacultyQuery)) *CourseQuery {
	query := &FacultyQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCourFacu = query
	return cq
}

//  WithCourDegr tells the query-builder to eager-loads the nodes that are connected to
// the "cour_degr" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CourseQuery) WithCourDegr(opts ...func(*DegreeQuery)) *CourseQuery {
	query := &DegreeQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCourDegr = query
	return cq
}

//  WithCourInst tells the query-builder to eager-loads the nodes that are connected to
// the "cour_inst" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CourseQuery) WithCourInst(opts ...func(*InstitutionQuery)) *CourseQuery {
	query := &InstitutionQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCourInst = query
	return cq
}
...
func (cq *CourseQuery) sqlAll(ctx context.Context) ([]*Course, error) {
	...
	if cq.withCourFacu != nil || cq.withCourDegr != nil || cq.withCourInst != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withCourFacu; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Course)
		for i := range nodes {
			if fk := nodes[i].faculty_facu_cour; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "faculty_facu_cour" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.CourFacu = n
			}
		}
	}

	if query := cq.withCourDegr; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Course)
		for i := range nodes {
			if fk := nodes[i].degree_degr_cour; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "degree_degr_cour" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.CourDegr = n
			}
		}
	}

	if query := cq.withCourInst; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Course)
		for i := range nodes {
			if fk := nodes[i].institution_inst_cour; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "institution_inst_cour" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.CourInst = n
			}
		}
	}
