...
// HasGendStud applies the HasEdge predicate on the "gend_stud" edge.
func HasGendStud() predicate.Gender {
	return predicate.Gender(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(GendStudTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, GendStudTable, GendStudColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasGendStudWith applies the HasEdge predicate on the "gend_stud" edge with a given conditions (other predicates).
func HasGendStudWith(preds ...predicate.Student) predicate.Gender {
	return predicate.Gender(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(GendStudInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, GendStudTable, GendStudColumn),
		)
