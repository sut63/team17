...
// QueryGendStud chains the current query on the gend_stud edge.
func (gq *GenderQuery) QueryGendStud() *StudentQuery {
	query := &StudentQuery{config: gq.config}
...
//  WithGendStud tells the query-builder to eager-loads the nodes that are connected to
// the "gend_stud" edge. The optional arguments used to configure the query builder of the edge.
func (gq *GenderQuery) WithGendStud(opts ...func(*StudentQuery)) *GenderQuery {
	query := &StudentQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withGendStud = query
	return gq
}
...
func (gq *GenderQuery) sqlAll(ctx context.Context) ([]*Gender, error) {
	...
	if query := gq.withGendStud; query != nil {
		...
		query.Where(predicate.Student(func(s *sql.Selector) {
			s.Where(sql.InValues(gender.GendStudColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.gender_gend_stud
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "gender_gend_stud" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "gender_gend_stud" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.GendStud = append(node.Edges.GendStud, n)
		}
	}
