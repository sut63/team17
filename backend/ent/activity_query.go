...
// QueryActiStud chains the current query on the acti_stud edge.
func (aq *ActivityQuery) QueryActiStud() *StudentQuery {
...
// QueryActiPlace chains the current query on the acti_place edge.
func (aq *ActivityQuery) QueryActiPlace() *PlaceQuery {
	query := &PlaceQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryActiAgen chains the current query on the acti_agen edge.
func (aq *ActivityQuery) QueryActiAgen() *AgencyQuery {
	query := &AgencyQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryActiYear chains the current query on the acti_year edge.
func (aq *ActivityQuery) QueryActiYear() *YearQuery {
...
//  WithActiStud tells the query-builder to eager-loads the nodes that are connected to
// the "acti_stud" edge. The optional arguments used to configure the query builder of the edge.
func (aq *ActivityQuery) WithActiStud(opts ...func(*StudentQuery)) *ActivityQuery {
	query := &StudentQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withActiStud = query
	return aq
}

//  WithActiPlace tells the query-builder to eager-loads the nodes that are connected to
// the "acti_place" edge. The optional arguments used to configure the query builder of the edge.
func (aq *ActivityQuery) WithActiPlace(opts ...func(*PlaceQuery)) *ActivityQuery {
	query := &PlaceQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withActiPlace = query
	return aq
}

//  WithActiAgen tells the query-builder to eager-loads the nodes that are connected to
// the "acti_agen" edge. The optional arguments used to configure the query builder of the edge.
func (aq *ActivityQuery) WithActiAgen(opts ...func(*AgencyQuery)) *ActivityQuery {
	query := &AgencyQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withActiAgen = query
	return aq
}

//  WithActiYear tells the query-builder to eager-loads the nodes that are connected to
// the "acti_year" edge. The optional arguments used to configure the query builder of the edge.
func (aq *ActivityQuery) WithActiYear(opts ...func(*YearQuery)) *ActivityQuery {
	query := &YearQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withActiYear = query
	return aq
}
...
//		GroupBy(activity.FieldACTIVITYNAME).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Activity.Query().
//		Select(activity.FieldACTIVITYNAME).
//		Scan(ctx, &v)
//
...
func (aq *ActivityQuery) sqlAll(ctx context.Context) ([]*Activity, error) {
	...
	if query := aq.withActiStud; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Activity)
		for i := range nodes {
			if fk := nodes[i].student_stud_acti; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "student_stud_acti" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ActiStud = n
			}
		}
	}

	if query := aq.withActiPlace; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Activity)
		for i := range nodes {
			if fk := nodes[i].place_place_acti; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "place_place_acti" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ActiPlace = n
			}
		}
	}

	if query := aq.withActiAgen; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Activity)
		for i := range nodes {
			if fk := nodes[i].agency_agen_acti; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(agency.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "agency_agen_acti" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ActiAgen = n
			}
		}
	}

	if query := aq.withActiYear; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Activity)
		for i := range nodes {
			if fk := nodes[i].year_year_acti; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "year_year_acti" returned %v`, n.ID)
			}
			for i := range nodes {
