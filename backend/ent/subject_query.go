...
// QuerySubjResu chains the current query on the subj_resu edge.
func (sq *SubjectQuery) QuerySubjResu() *ResultsQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(subject.Table, subject.FieldID, sq.sqlQuery()),
...
// First returns the first Subject entity in the query. Returns *NotFoundError when no subject was found.
...
// FirstID returns the first Subject id in the query. Returns *NotFoundError when no id was found.
...
func (sq *SubjectQuery) FirstXID(ctx context.Context) int {
...
// Only returns the only Subject entity in the query, returns an error if not exactly one entity was returned.
...
// OnlyID returns the only Subject id in the query, returns an error if not exactly one id was returned.
...
//  WithSubjResu tells the query-builder to eager-loads the nodes that are connected to
// the "subj_resu" edge. The optional arguments used to configure the query builder of the edge.
func (sq *SubjectQuery) WithSubjResu(opts ...func(*ResultsQuery)) *SubjectQuery {
	query := &ResultsQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withSubjResu = query
	return sq
}
...
//		Scan(ctx, &v)
//
func (sq *SubjectQuery) Select(field string, fields ...string) *SubjectSelect {
	selector := &SubjectSelect{config: sq.config}
...
func (sq *SubjectQuery) sqlAll(ctx context.Context) ([]*Subject, error) {
	...
	if query := sq.withSubjResu; query != nil {
		...
		query.Where(predicate.Results(func(s *sql.Selector) {
			s.Where(sql.InValues(subject.SubjResuColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.subject_subj_resu
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "subject_subj_resu" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "subject_subj_resu" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.SubjResu = append(node.Edges.SubjResu, n)
		}
	}
...
func (sq *SubjectQuery) sqlQuery() *sql.Selector {
	builder := sql.Dialect(sq.driver.Dialect())
	t1 := builder.Table(subject.Table)
	selector := builder.Select(t1.Columns(subject.Columns...)...).From(t1)
	if sq.sql != nil {
		selector = sq.sql
		selector.Select(selector.Columns(subject.Columns...)...)
	}
...
// SubjectGroupBy is the builder for group-by Subject entities.
...
// SubjectSelect is the builder for select fields of Subject entities.
...
func (ss *SubjectSelect) sqlQuery() sql.Querier {
	selector := ss.sql
